describe('Basic user flow for Website', () => {
  ...
  // shadowRoot of that product item
  const shadowRoot = await productHandle.getProperty('shadowRoot');

  // From shadowRoot, query the button inside
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    await page.waitForFunction(() => {
      ...
      return Array.from(items).every(item => item.shadowRoot && item.shadowRoot.querySelector('button'));
    });
    ...
    // After clicking all buttons, check the cart count
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
     ...
     * Remember to remove the .skip from this it once you are finished writing this test.
     */
      // Evaluate code in the browser context to get localStorage data
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    // Only click if it still says "Remove from Cart"
      if (buttonText === "Remove from Cart") {
        await button.click();
        await new Promise(resolve => setTimeout(resolve, 50));
        //await page.waitForTimeout(50); 
